...
class DapPhase3_v5(DapPhase2_v5):
    ...
    def phase3(self, input_mix, input_age, input_gender, input_weight):
        ...
        max_reach = df.groupby(['campaign', 'gender', 'age_min', 'age_max']).\
            agg(
        ...
        ovr = df.groupby(['campaign', 'gender', 'age_min', 'age_max']). \
        ...
        grouped = df.groupby(['campaign', 'gender', 'age_min', 'age_max'])
        trans_list = [[]]

        for i in range(ovr.shape[0]):
            key = (ovr['campaign'][i], ovr['gender'][i], ovr['age_min'][i], ovr['age_max'][i])
            try:
        ...
        ovr['af'] = ovr['eimp_weighted'] / ovr['e_reach_n']
        ovr['af_a'] = ovr['e_imp_a'] / ovr['e_reach_n']
    ...
    def summary_each_campaign(self, input_mix, input_age, input_gender, input_weight):
        ...
        df = pd.merge(df, mix_cleaned.groupby(['campaign']). \
        ...
        df = df.drop('period', axis=1).assign(line="Campaign Total", platform=None, product=None, gender=None, age_min=None,
                                            age_max=None)

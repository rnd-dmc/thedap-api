...
class getDATA():
    ...
    def getPopulation(self):
        ...
        # df = pd.read_excel('C:\\Users\\user\\OneDrive\\Desktop\\데이터마케팅\\효과예측 (외부)\\thedap_module\\thedap_250903\\src\\CONFIG\\population_DB_sample.xlsx')
        # df = df.groupby(['gender', 'age_min', 'age_max']).agg(date = ('date', 'max')).reset_index().\
        #     merge(df).assign(year = None, month = None)
    ...
    def getDistribution(self):
        ...
        # 연결 종료
        cnx.close()
        # df = pd.read_excel('C:\\Users\\user\\OneDrive\\Desktop\\데이터마케팅\\효과예측 (외부)\\thedap_module\\thedap_250903\\src\\CONFIG\\distribution_DB_sample.xlsx')
        # df = df.groupby(['platform', 'gender', 'age_min', 'age_max']).agg(date = ('date', 'max')).reset_index().\
        #     merge(df).assign(year = None, month = None)
    ...
    def getParameter(self):
        ...
        # 연결 종료
        cnx.close()
        # df = pd.read_excel('C:\\Users\\user\\OneDrive\\Desktop\\데이터마케팅\\효과예측 (외부)\\thedap_module\\thedap_250903\\src\\CONFIG\\parameter_DB_sample.xlsx')
        # df = df.groupby(['platform', 'product', 'gender', 'age_min', 'age_max']).agg(date = ('date', 'max')).reset_index().\
        #     merge(df).assign(year = None, month = None)
            
        subttl = df.query('product == "overall"').assign(product = 'subtotal')
    ...
    def getNPlusParameter(self):
        ...
        # 연결 종료
        cnx.close()
        # df = pd.read_excel('C:\\Users\\user\\OneDrive\\Desktop\\데이터마케팅\\효과예측 (외부)\\thedap_module\\thedap_250903\\src\\CONFIG\\parameter_nplus_DB_sample.xlsx')
        # df = self.getPopulation().merge(df.drop('date', axis=1), how='cross').assign(year = None)

...
def makeReportAnalysis(result_json):
    ...
    mix_xl = pd.DataFrame(result_json['mix_analysis'])
    ...
    ws2.cell(row=4, column=3, value=f"{option_analysis['input_gender']}{option_analysis['input_agemin']}{option_analysis['input_agemax']}").style = title_style

    ws2.cell(row=5, column=2, value="타겟 모수").style = index_style
    ws2.cell(row=5, column=3, value=result_json['population']).style = title_style

    ws2.cell(row=6, column=2, value="데이터 기준").style = index_style
    ws2.cell(row=6, column=3, value=option_analysis['maxdate']).style = title_style
    ...
    summary_xl = pd.DataFrame(result_json['result_summary']).\
        filter(
            [
    ...
    ws2.cell(row=row1, column=9, value="도달 분포")
    ...
    freq_xl = pd.DataFrame(result_json['reach_freq'])
    ...
    for k in [j.keys() for j in result_json['heatmap']][0]:
        stand_ = result_json['heatmap'][0][k]
    ...
    metric_validator = DataValidation(
        type="list", formula1=f"= RAW_DATA!E2:Y2",
        allow_blank=True, showErrorMessage=True, showInputMessage=True
    )
    ...
    color_rule = ColorScaleRule(
        ...
        end_type='formula', end_value="RAW_DATA!$Z$1", end_color="F8696B"
    )
